...
export async function handleGenerate(body = {}, userId = "local") {
  try {
    ...
    // Edit mode: image attached to prompt area → pure editing, refs ignored.
    ...
    // Edit mode: only the edit image is sent as reference (no role zones).
    // Normal mode: face -> pose -> garment -> product -> background.
    ...
    let finalPrompt;
    if (isEditMode) {
      // Pure editing: the user's text IS the edit instruction, no anchors.
      finalPrompt = core;
    } else if (mode === "auto") {
      finalPrompt =
    ...
    // Edit mode always uses draft tier (Seedream Edit) regardless of selector.
    const effTier = isEditMode ? "draft" : (tier || "draft");
