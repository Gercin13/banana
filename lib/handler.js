...
export async function handleGenerate(body = {}, userId = "local") {
  try {
    ...
    // Engine caps refs at 14; if the combined set exceeds that, trim gracefully.
    const refs = [...faceRefs, ...poseRefs, ...garmentRefs, ...productRefs, ...backgroundRefs].slice(0, MAX_REFS);
    ...
    // Background default: with no background reference, keep a plain white
    // background unless the prompt describes a specific setting.
    ...
    const effSize = tier === "lite" ? "1K" : (size || "1K"); // Lite supports 1K only
