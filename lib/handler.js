...
export async function handleGenerate(body = {}, userId = "local") {
  try {
    ...
    const totalRefs = faceRefs.length + poseRefs.length + garmentRefs.length + productRefs.length;
    ...
    const refs = [...faceRefs, ...poseRefs, ...garmentRefs, ...productRefs].slice(0, MAX_REFS);
    ...
    const record = saveRecord({
      ...
      refs: { face: faceRefs.length, pose: poseRefs.length, garment: garmentRefs.length, product: productRefs.length },
      aspectRatio: aspectRatio || "1:1",
      size: size || "1K",
      tier: tier || "draft",
      model: result.model,
