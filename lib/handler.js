...
import { generate, generateVideo, generateOneNvidia, upscaleImage, upscaleVideo, MODELS, TIERS, VIDEO_MODELS, ASPECT_RATIOS, IMAGE_SIZES, MAX_IMAGES, NVIDIA_RATIOS, NVIDIA_MODES } from "./engine.js";
import {
...
export function healthPayload() {
  return {
    ...
    videoModels: Object.fromEntries(Object.entries(VIDEO_MODELS).map(([k, v]) => [k, { label: v.label, supportsResolution: v.supportsResolution, supportsLastFrame: v.supportsLastFrame, supportsAudio: v.supportsAudio, durationOptions: v.durationOptions }])),
...
export async function handleGenerate(body = {}, userId = "local") {
  try {
    ...
    // editImages: array (multi-angle refs uploaded via the prompt area 📎).
    // Backward compat: also accept legacy single `editImage`.
    const editImages = cleanRefs(
      Array.isArray(body.editImages) ? body.editImages :
      (body.editImage ? [body.editImage] : [])
    );
    ...
    // Edit mode: one or more images attached via 📎. The 5 ref zones ALSO apply.
    ...
    // editImages go first (all angles of input subject), then zone refs.
    ...
    let finalPrompt;
    if (isEditMode) {
      // Edit mode: use uploaded reference images to generate a consistent result.
      const refNote = editImages.length > 1
        ? `Use the ${editImages.length} provided reference images (different angles of the same subject) to generate a maximally consistent and accurate depiction.`
        : "Use the provided reference image.";
      finalPrompt = anchorText
        ? `${refNote} ${capitalize(anchorText)}. ${core}`
        : `${refNote} ${core}`;
    } else if (mode === "auto") {
      finalPrompt =
    ...
    const nvidiaInputImage = (effTier === "nvidia" && nvidiaMode && nvidiaMode !== "base" && editImages.length)
      ? editImages[0] : null;
...
export async function handleGenerateVideo(body = {}, userId = "local") {
  try {
    ...
    const dur = Number(duration) >= 3 && Number(duration) <= 10 ? Number(duration) : 5;
    const modelKey = videoModelKey || "wan22";
    ...
    const result = await generateVideo({
      ...
      duration: dur,
      seed: seed !== undefined ? Number(seed) : -1,
