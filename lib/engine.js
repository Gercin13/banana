...
export const TIERS = Object.keys(MODELS);
...
// ---- Client-polling helpers (for Vercel serverless) -----------------------
// submitWavespeed: sends task to WaveSpeed, returns { taskId, resultUrl }.
// checkWavespeed: polls resultUrl once, returns { status, outputs? }.
export async function submitWavespeed(modelSlug, body) {
  const task = await submit(modelSlug, body);
  return { taskId: task.id, resultUrl: task.urls?.get || `${BASE}/predictions/${task.id}/result` };
}

export async function checkWavespeed(resultUrl) {
  const resp = await fetch(resultUrl, { headers: { Authorization: `Bearer ${apiKey()}` } });
  if (!resp.ok) throw new Error(`WaveSpeed poll ${resp.status}`);
  const json = await resp.json();
  const data = json.data || {};
  return { status: data.status, outputs: data.outputs || [], error: data.error };
}
