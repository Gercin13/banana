...
async function generateOne({ prompt, aspectRatio, model, refs = [] }) {
...
export async function generate({ prompt, aspectRatio = "1:1", count = 1, tier = "draft", refs = [] }) {
  ...
  const settled = await Promise.allSettled(
    Array.from({ length: n }, () => generateOne({ prompt, aspectRatio, model, refs: safeRefs }))
  );
