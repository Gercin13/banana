...
// Save a character. `images` = [{ dataBase64, mimeType }]. Returns the record.
export function saveCharacter({ name, images }) {
  const id = crypto.randomUUID();
  const saved = (images || []).map((im) => saveImage(im)); // {id,url,file,mimeType}
  const record = {
    id,
    name: String(name || "").slice(0, 100),
    createdAt: new Date().toISOString(),
    images: saved.map(({ url, file, mimeType }) => ({ url, file, mimeType })),
  };
  fs.writeFileSync(path.join(CHARACTERS_DIR, `${id}.json`), JSON.stringify(record));
  return record;
}
...
export function listCharacters() {
  ...
  for (const f of fs.readdirSync(CHARACTERS_DIR)) {
    if (!f.endsWith(".json")) continue;
    try { out.push(JSON.parse(fs.readFileSync(path.join(CHARACTERS_DIR, f), "utf8"))); } catch { /* skip */ }
  }
...
export function deleteCharacter(id) {
  const p = path.join(CHARACTERS_DIR, `${path.basename(String(id))}.json`);
  if (!fs.existsSync(p)) return false;
  try {
    const rec = JSON.parse(fs.readFileSync(p, "utf8"));
    for (const im of rec.images || []) {
      if (!im.file) continue;
      const fp = path.join(IMAGES_DIR, path.basename(im.file));
...
// Load a character's face images as inline refs [{ mimeType, dataBase64 }].
export function loadCharacterRefs(id) {
  const p = path.join(CHARACTERS_DIR, `${path.basename(String(id))}.json`);
  if (!fs.existsSync(p)) return [];
  let rec;
  try { rec = JSON.parse(fs.readFileSync(p, "utf8")); } catch { return []; }
  const refs = [];
  for (const im of rec.images || []) {
    try {
      const data = fs.readFileSync(path.join(IMAGES_DIR, path.basename(im.file))).toString("base64");
      refs.push({ mimeType: im.mimeType || "image/jpeg", dataBase64: data });
